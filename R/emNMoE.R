...
#'
#' nmoe$plot()
emNMoE <- function(X, Y, K, p = 3, q = 1, n_tries = 1, max_iter = 1500,
                   threshold = 1e-6, verbose = FALSE, verbose_IRLS = FALSE, update_IRLS = TRUE,
                   nearTrue = FALSE, alphak_0 = NA, betak_0 = NA, sigmak_0 = NA) {
  ...
  while (try_EM < n_tries) {
    ...
    param$initParam(segmental = FALSE, nearTrue, alphak_0, betak_0, sigmak_0)
