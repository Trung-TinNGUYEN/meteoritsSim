...
// [[Rcpp::export]]
List IRLS(arma::mat& X, arma::mat& Tau, arma::mat& Gamma, arma::mat& Winit, bool verbose = false, bool updateIRLS = true) {
  ...
  //            i=1,...n,j=1...m,  k=1,...,K,
  //            * vi = [1,ti,ti^2,...,ti^q]^T;
  //       The parameter vectors are in the matrix W=[w1,...,wK] (with wK is the null vector);
  //
  ...
  double lambda = 0; // if a MAP regularization (a gaussian prior on W) (L_2 penalization); lambda is a positive hyperparameter
  // Test without MAP regularization.
  ...
  while (not(converge) && (iter < max_iter)) {
    ...
    // // Hw_old = Gradient 2 Test.
    // arma::mat Hw_old_grad(hx, hx, arma::fill::ones);
    ...
    if (updateIRLS == true) {
      ...
      w = vectorise(W_old, 0) -  1e-3 * gw_old; // [(q+1)x(K-1),1]
    }
    // w = vectorise(W_old, 0) - arma::inv(Hw_old) * gw_old; // [(q+1)x(K-1),1]
    // Rcout << "IRLS : Iteration : "  + std::to_string(iter) + "; nrow = ncol = " + std::to_string(hx)  + "; Hw_old_max: " + std::to_string((arma::inv(Hw_old)).max()) + "; Hw_old_min: " + std::to_string((arma::inv(Hw_old)).min())+ "\n";
    // Test code Tin
    // Using simple gradient descent when We initialized the EM algorithm favourably by making
    // a partition of starting values near the true components.
    // w = vectorise(W_old, 0) -  1e-3 * gw_old; // [(q+1)x(K-1),1]

    // // Test: Print some values.
    // // Test
    // Rcout << "vectorise(W_old, 0) = \n";
    // (vectorise(W_old, 0)).print();
    // Rcout << "w = \n";
    // w.print();
    // Rcout << "0.01 * gw_old = \n";
    // (1e-9 * gw_old).print();
    // Rcout << "\n";
    // Rcout << "(arma::inv(Hw_old) * gw_old) = \n";
    // (arma::inv(Hw_old) * gw_old).print();
    // Rcout << "\n";
    // Rcout << "(0.5 * Hw_old_grad * gw_old) = \n";
    // (0.5 * Hw_old_grad * gw_old).print();
    // Rcout << "\n";
    // Rcout << "inv(Hw_old) = \n";
    // arma::inv(Hw_old).print();
    // Rcout << "\n";
    ////
    // Rcout << "IRLS : Iteration : " + std::to_string(iter) + "; nrow = ncol = " + std::to_string(hx) + "; Hw_old_grad_max: " + std::to_string((0.5 *Hw_old_grad).max()) + "; Hw_old_grad_min: " + std::to_string((Hw_old_grad).min())+ "\n";
    ...
    // // Test
    // Rcout << "IRLS : Iteration : "  + std::to_string(iter) + "; Log-lik: " + std::to_string(loglik) + "\n";
    ...
    double stepsize = 1; // Initialization, no adaptation of the Newton Raphson algorithm.
    double alpha = 2;
    while (loglik < loglik_old) {
      ...
      // Newton Raphson : W(t+1) = W(t) - H(W(t))^(-1)g(W(t))
      if (updateIRLS == true) {
        w = vectorise(W_old, 0) - stepsize * arma::inv(Hw_old) * gw_old; // [(q+1)x(K-1),1]
      } else{
        w = vectorise(W_old, 0) -  stepsize * 1e-3 * gw_old; // [(q+1)x(K-1),1]
      }
      ...
      // Rcout << "w_check = \n";
      // w.print();
      // Rcout <<  "Log-lik_check = : " + std::to_string(loglik) + "\n";
      // Rcout <<  "Log-lik_old_check = : " + std::to_string(loglik_old) + "\n";
      // Rcout <<  "loglik < loglik_old =  " + std::to_string(loglik < loglik_old) + "Log-lik_check = : " + std::to_string(loglik) + "\n";
